          ...
          <div key={filament.id} className="bg-gray-800 rounded-xl border border-gray-700 p-6 hover:border-cyan-500/50 transition-colors">
            ...
            <div className="space-y-2 text-sm text-gray-300 mb-6">
              <div className="flex justify-between">
                <span>Preço do Rolo:</span>
                <span className="font-medium text-white">R$ {filament.roll_price.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span>Estoque (Rolos):</span>
                <span className="font-medium text-white">{filament.rolls || 0}</span>
              </div>
              <div className="flex justify-between">
                <span>Estoque (Gramas):</span>
                <span className={`font-medium ${filament.current_weight_g && filament.current_weight_g < filament.min_stock_alert_g ? 'text-red-500' : 'text-white'}`}>
                  {filament.current_weight_g?.toFixed(0)}g
                </span>
              </div>
              <div className="flex justify-between">
                <span>Custo/g:</span>
                <span className="font-medium text-cyan-400">R$ {(filament.roll_price / filament.roll_weight_g).toFixed(4)}</span>
              </div>
            </div>

            <div className="flex gap-2">
              <button
                onClick={() => openPurchaseModal(filament)}
                className="flex items-center justify-center px-3 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors"
