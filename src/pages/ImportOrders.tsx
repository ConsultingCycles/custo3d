...
export const ImportOrders = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        // Chama o tradutor que criamos
        const { valid } = await parseShopeeExcel(e.target.files[0], products);
        setPreviewData(valid);
      } catch (err) {
        alert('Erro ao ler arquivo. Verifique se é um relatório da Shopee válido.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleImport = async () => {
    if (!selectedMarketplace) return alert('Selecione o canal de venda');
    setProcessing(true);
    await importOrdersBatch(previewData, selectedMarketplace);
    setProcessing(false);
    alert('Importação concluída! Os pedidos novos foram adicionados.');
    navigate('/orders');
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      ...
      <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm text-gray-400 mb-2">1. Canal de Venda (Destino)</label>
            <select 
                    ...
                    <td className="p-3">
                      {order.items.map((i: any, idx: number) => (
                        <div key={idx} className="flex gap-2 text-xs">
                          {/* Verifica se o SKU existe na lista de produtos carregada */}
